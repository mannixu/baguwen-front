        ...
        <view class="scroll-content">
          ...
          <view
            ...
            :style="{
			  ...
			  'margin-left': defaultConfig.marginleft+'rpx'   //含有特殊字符如- 加上''
...
export default {
  name: 'my-tabs',
  props: {
    // 父组件传入的 tabs 数据 (父组件传递的数据不应该在子组件修改 可以通过在data设置个变量 把父组件传过来值赋给变量 然后修改变量)
    tabData: {
      type: Array,
      default: () => []
    },
  ...
  watch: {
    ...
    config: {
      handler(val) {
        this.defaultConfig = { ...this.defaultConfig, ...val };  //(把穿过来的val与前面defautlconfig合并相同后面覆盖前面的)
      },
      // 该回调将会在侦听开始之后被立即调用
      immediate: true
    }
  ...
  methods: {
    ...
    tabClick(index) {
      ...
      // 定义滑块的位置
      this.tabToIndex();
      // 发送通知  (向父组件发生通知 因为点击事件在组件内 要把修改的tab传递给list进行数据展示 由此组件传给父组件hot然后父组件再传给子组件list)
