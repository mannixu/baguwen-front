<template>
	<view>
		...
		<f-audio v-else-if="detail.type == 'audio'" :list="detail" :src="detail.content" @onProgress="onAudioProgressUpdate"></f-audio>
		...
		<view class="animate__animated animate__fadeIn animate__faster">
			<view v-if="firstLoad" class="flex flex-column p-3">
				...
				<view v-if="!detail.isbuy" class="flex mt-2 align-end">
					...
					<text class="text-danger font-lg">￥{{activeData?activeData.data.price: detail.price }}</text>
